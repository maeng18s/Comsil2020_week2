...
void Print_Line( char *line, int *Count, int *B_Flag ) {
	int i, N_Blanks, N_Chars, Start = 0;	//start 초기화

	Get_Blanks_Chars( line, Start, &N_Blanks, &N_Chars );//줄을 읽어옴

	//N_Chars = 0 될때까지 반복(N_Chars=다음단어의 글자수)
	while ( N_Chars != 0 ) {
		if ( (*B_Flag == 1) && ((*Count + N_Chars + 1) <= LIMIT) ) {
			...
			*B_Flag = 0;
			...
			for ( i = Start; i < Start + N_Chars ; i++ ) {
				//L2[Start]-L2[Start+N+Chars-1]
				putchar(line[i]);
			}

			Start = Start + N_Chars;
			*Count = *Count + N_Chars + 1;	//limit 계산
